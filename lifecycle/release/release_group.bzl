"""`farakov_release_group` — build-time aggregation of push targets.
    ...
    if not ctx.attr.allow_duplicate_components:
        ...
        for p in pushes:
            if p.component_name in seen:
                fail(
                    "farakov_release_group: duplicate component_name %r. " % p.component_name +
