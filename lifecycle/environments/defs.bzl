"""Public factory macro: `coen_applied_lifecycle_environments`.
...
replaces all of them. Downstream rules in rules_coen_applied_lifecycle NEVER
...
def _validate_env_name(name):
    ...
    if not name:
        fail("coen_applied_lifecycle_environments: environment names must be non-empty.")
    if name != name.lower():
        fail("coen_applied_lifecycle_environments: environment '%s' must be lowercase." % name)
    allowed = "abcdefghijklmnopqrstuvwxyz0123456789-"
    for ch in name.elems():
        if ch not in allowed:
            fail(
                "coen_applied_lifecycle_environments: environment name '%s' " % name +
                "contains invalid character %r. Allowed: [a-z0-9-]." % ch,
            )
    if name[0] == "-" or name[-1] == "-":
        fail("coen_applied_lifecycle_environments: environment '%s' must not start or end with '-'." % name)

def coen_applied_lifecycle_environments(
        environments,
        default_environment,
        default_image_tag = "latest",
    ...
    if type(environments) != "list":
        fail("coen_applied_lifecycle_environments: `environments` must be a list, got %s." % type(environments))
    if not environments:
        fail("coen_applied_lifecycle_environments: `environments` must be non-empty.")
    ...
    if default_environment not in envs:
        fail(
            "coen_applied_lifecycle_environments: default_environment='%s' " % default_environment +
