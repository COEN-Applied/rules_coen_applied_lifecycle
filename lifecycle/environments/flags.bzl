"""Build-setting rules backing `farakov_lifecycle_environments`.
...
`farakov_lifecycle_environments` factory macro — this is how we avoid the
...
def _lifecycle_environment_impl(ctx):
    ...
    if not allowed:
        fail(
            ...
            "farakov_lifecycle_environments(environments=[...]) with an " +
